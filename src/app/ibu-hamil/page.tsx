...
import { Plus, Trash2, Activity, Eye, Heart } from 'lucide-react';
import { Warga, PengukuranIbuHamil } from '@/types';
...
export default function IbuHamilPage() {
  ...
  const [pengukuranList, setPengukuranList] = useState<PengukuranIbuHamil[]>([]);
  ...
  const handleDeleteWarga = async (id: string) => {
    if (!confirm('Hapus data ibu hamil ini? Data pengukuran terkait juga akan terhapus.')) return;
    try {
    ...
    <div className="p-6 max-w-7xl mx-auto">
      <div className="flex flex-col md:flex-row justify-between items-center mb-8 gap-4">
        <div>
          ...
          <p className="text-gray-600">Total: <strong>{wargaList.length}</strong> ibu hamil terdaftar</p>
        </div>
        <button
      ...
      <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-6">
        {wargaList.length === 0 ? (
          <div className="col-span-full text-center py-12 bg-white rounded-xl border border-dashed border-gray-300">
            ...
            <p className="text-gray-500 text-lg">Belum ada data ibu hamil</p>
          </div>
        ) : (
          wargaList.map((warga) => {
            ...
            return (
              <div key={warga.id} className="bg-white rounded-xl shadow-sm border border-gray-100 hover:shadow-lg transition-all overflow-hidden group">
                <div className="p-5 border-b border-gray-50 bg-gradient-to-r from-pink-50 to-white">
                ...
                <div className="p-5 space-y-4">
                  <div className="grid grid-cols-2 gap-4 text-sm">
                    <div>
                      <p className="text-gray-400 text-xs uppercase tracking-wider mb-1">Alamat</p>
                      <p className="font-medium text-gray-700 truncate">{warga.alamat || '-'}</p>
                    </div>
                  </div>
                    ...
                    <div className="bg-gray-50 rounded-lg p-3 border border-gray-100">
                      ...
                      <div className="flex justify-between items-end">
                        ...
                        <div className="text-right">
                          <p className="text-xs text-gray-500">Usia Hamil</p>
                          <p className="font-semibold text-gray-800">{latestPengukuran.usia_kehamilan} minggu</p>
                        </div>
                      </div>
                    </div>
                  ) : (
                  ...
                  {showDetail === warga.id && latestPengukuran && (
                    <div className="pt-3 border-t border-gray-100 grid grid-cols-2 gap-y-3 gap-x-2 text-sm animate-in fade-in slide-in-from-top-1">
                      <div><p className="text-xs text-gray-500">Tensi</p><p className="font-medium">{latestPengukuran.tekanan_darah_sistolik}/{latestPengukuran.tekanan_darah_diastolik}</p></div>
                      <div><p className="text-xs text-gray-500">Hb</p><p className="font-medium">{latestPengukuran.hb || '-'} g/dL</p></div>
                ...
                <div className="px-5 py-4 bg-gray-50 flex gap-2 border-t border-gray-100">
                  ...
                  <button
                    ...
                    className="flex-1 flex items-center justify-center gap-1 text-white bg-pink-600 hover:bg-pink-700 py-2 rounded-lg text-sm shadow-sm transition-all"
                  >
        ...
        <div className="fixed inset-0 bg-black/60 backdrop-blur-sm flex items-center justify-center p-4 z-50 animate-in fade-in duration-200">
          <div className="bg-white rounded-2xl w-full max-w-2xl max-h-[90vh] overflow-y-auto">
            <div className="sticky top-0 bg-white p-4 border-b flex justify-between items-center z-10">
              <h3 className="font-bold text-lg">Tambah Data Ibu Hamil</h3>
              <button onClick={() => setShowFormWarga(false)} className="p-2 hover:bg-gray-100 rounded-full">✕</button>
            </div>
            <FormWarga onSuccess={handleSuccessWarga} onCancel={() => setShowFormWarga(false)} initialKategori="ibu_hamil" />
          </div>
        </div>
      )}
